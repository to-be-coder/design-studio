...
function structurePrompt({
  ...
  return [
    ...
    `- CASE 2, path present AND a PRISTINE skeleton (git working tree clean via \`git -C ${targetRepo} status --porcelain\` empty, AND every commit is structure's own so \`git -C ${targetRepo} log --oneline\` shows ONLY \`Skeleton scaffold\` / \`Skeleton re-scaffold\` lines, AND ${targetRepo}/flows.json source is "structure"): REFRESH it. Regenerate every skeleton file in place from the CURRENT inputs, add one \`Skeleton re-scaffold\` commit, keep source "structure", and leave prototype_repo as is. Repeated refreshes stay pristine (each just adds another re-scaffold commit).`,
    `- CASE 3, path present but NOT pristine (dirty tree, a non-structure commit like design-system's DESIGN.md or a build commit, or flows.json source "build"): TOUCH NOTHING inside it. Append ONE dated plain line to 00 Dashboard.md naming the conflict (that ${targetRepo} has work in it, so a refresh would clobber it), and do not scaffold. Stop after the note.`,
    "",
