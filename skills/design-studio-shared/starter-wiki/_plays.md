...
- **"An identity or system already exists and we're adding to it"** → [[Derive before invent]]
- **"Many agents (or sessions) will build parts of one thing"** →
  [[One contract for parallel agents]]
- **"A human must choose between abstract options (tokens, configs, schemas)"** →
  [[Specimen boards]]
- ⚠ **"We're defining a new format, template, or contract"** →
